...
_SHARDCACHE_HASH_KEY = None


class _ScnpConnection:
    ...
    def _read_get_response(self) -> Optional[bytes]:
        ...
        raise RuntimeError(f"GET unexpected SCNP response status: {status}")
    ...
    def _read_set_response(self) -> None:
        ...
        if status == _STATUS_ERROR:
            raise RuntimeError(self._read_exact(body_len).decode("utf-8", "replace"))
        raise RuntimeError(f"SET unexpected SCNP response status: {status}")
    ...
    def _send_parts(self, parts: Iterable[bytes | memoryview]) -> None:
        ...
        while views:
            ...
            if sent == 0:
                raise ConnectionError("SCNP connection closed while writing request")
    ...
    def _read_response_header(self) -> tuple[int, int]:
        ...
        if magic != _FAST_RESPONSE_MAGIC:
            raise RuntimeError(f"bad SCNP response magic: 0x{magic:02x}")
        if version != _FAST_PROTOCOL_VERSION:
            raise RuntimeError(f"bad SCNP response version: {version}")
    ...
    def _read_exact(self, size: int) -> bytearray:
        ...
        while remaining:
            ...
            if read == 0:
                raise ConnectionError("SCNP connection closed while reading response")
            offset += read
            remaining -= read
        return out
    ...
    def _discard(self, size: int) -> None:
        ...
        while remaining:
            ...
            if not chunk:
                raise ConnectionError("SCNP connection closed while discarding response")
            remaining -= len(chunk)


class ShardCacheScnpStore:
    """Minimal shardcache Store-compatible adapter over SCNP/TCP.
    ...
    def set(self, key: bytes, value: bytes, ttl: Optional[int] = None) -> None:
        if ttl is not None:
            raise NotImplementedError("SCNP TCP LMCache adapter does not support TTL")
    ...
    def delete(self, key: bytes) -> bool:
        raise NotImplementedError("SCNP TCP LMCache adapter does not support DELETE yet")

    def _conn(self) -> _ScnpConnection:
        conn = getattr(self._local, "conn", None)
        if conn is None:
            conn = _ScnpConnection(self._addr)
            self._local.conn = conn
        return conn


def create_shardcache_store(*, addr: str) -> ShardCacheScnpStore:
    return ShardCacheScnpStore(addr)


def _hash_key(key: bytes) -> int:
    global _SHARDCACHE_HASH_KEY
    if _SHARDCACHE_HASH_KEY is None:
        try:
            import shardcache  # type: ignore[import-not-found]
        except ImportError as exc:
            raise RuntimeError("shardcache module is required for SCNP key hashing") from exc

        try:
            _SHARDCACHE_HASH_KEY = shardcache.hash_key
        except AttributeError as exc:
            raise RuntimeError(
                "shardcache.hash_key is required for optimized SCNP/TCP frames; "
                "reinstall the shardcache Python extension from this checkout"
            ) from exc

    return int(_SHARDCACHE_HASH_KEY(key))


def _parse_addr(addr: str) -> tuple[str, int]:
    host, sep, port_text = addr.rpartition(":")
    if not sep or not host:
        raise ValueError(f"SCNP address must be host:port, got {addr!r}")
