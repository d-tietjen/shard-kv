"""LMCache storage backend implemented on top of ``shardcache.Store``.

The wrapper intentionally keeps the moving pieces small:

- serialize a ``MemoryObj`` into one binary value for shardcache
...
import shardcache
import torch
from shardcache_local_embedded_store import create_shardcache_store
...
class _StoredRecord:
    """Decoded payload stored in shardcache."""
...
class ShardCacheStorageBackend(StoragePluginInterface):
    """LMCache storage plugin backed by the local shardcache embedded store.
    ...
    def __init__(
        ...
        **_: Any,
        ...
        self._scnp_addr = str(self._get_config_value("scnp_addr", "127.0.0.1:6500")).strip()
        ...
        self._metrics_artifacts_dir = str(
            self._get_config_value(
                ...
                os.environ.get("SHARDCACHE_METRICS_DIR", ""),
            )
            or ""
        ).strip()
        self._store = create_shardcache_store(
            cores=self._cores,
        ...
        self._executor = ThreadPoolExecutor(
            ...
            thread_name_prefix="shardcache-lmcache",
        )
    ...
    def get_allocator_backend(self) -> Any:
        if self.local_cpu_backend is None:
            raise RuntimeError(
                "ShardCacheStorageBackend requires local_cpu_backend for MemoryObj reconstruction"
            )
    ...
    def touch_cache(self) -> None:
        # shardcache has no separate eviction policy for this LMCache wrapper, so
    ...
    def _detect_config_prefix(self) -> str:
        ...
        class_names = {self.__class__.__name__, "FastCacheStorageBackend"}
        for key, value in extra.items():
            if not key.endswith(".module_path") or value != module_path:
                continue
            prefix = key[: -len(".module_path")]
            if extra.get(f"{prefix}.class_name") in class_names:
                return prefix
        if any(key.startswith("storage_plugin.fast_cache.") for key in extra):
            return "storage_plugin.fast_cache"
        # Fall back to the documented example name.
        return "storage_plugin.shardcache"
        ...
        match self._connection:
            case "embedded" | "in_process" | "local":
                if architecture in {"shared", "local_embedded", "embedded", "in_process", "local"}:
                    return _normalize_client_architecture(architecture)
                return "local_embedded"
            case "tcp" | "scnp_tcp" | "remote":
                if architecture in {"scnp_tcp_python", "tcp_python"}:
                    return "scnp_tcp_python"
                return "scnp_tcp"
            case "tcp_python":
                return "scnp_tcp_python"
            case other:
                raise ValueError(
                    "unsupported shardcache LMCache connection "
    ...
    def _dump_metrics_artifacts_safe(self) -> None:
        try:
            ...
            logger.warning("failed to dump shardcache telemetry artifacts: %s", exc)
    ...
    def _decode_record(self, raw_value: Any) -> _StoredRecord:
        ...
        raise ValueError("shardcache LMCache record is missing the expected header")
