...
fn configured_handoff_buffer_max() -> usize {
    ...
    *VALUE.get_or_init(|| {
        std::env::var("SHARDCACHE_HANDOFF_BUFFER_BYTES")
            .or_else(|_| std::env::var("SCNP_HANDOFF_BUFFER_BYTES"))
            .ok()
...
impl WriteSubmitter {
    ...
    pub(super) async fn submit(handoff: &WriteHandoff, buf: Vec<u8>) -> Result<()> {
        ...
        handoff
            ...
            .await
            .map_err(|error| {
                crate::ShardCacheError::Protocol(format!("write handoff error: {error}"))
            })?;
        Ok(())
    }
}
