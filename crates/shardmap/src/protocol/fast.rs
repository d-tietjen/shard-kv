...
/// Native shardcache protocol, version 2.
///
...
/// opcodes can coexist with older typed SCNP opcodes. RESP remains supported
...
pub enum FastCommand<'a> {
    ...
    /// Generic SCNP wrapper for the Redis command surface.
    ///
    ...
    /// SCNP value so clients can use one native ingress path for the complete
    /// compatibility surface while hot commands keep their typed opcodes.
    RespCommand {
        parts: Vec<&'a [u8]>,
    },
    /// Opcode-specific SCNP wrapper for Redis commands without a typed body.
    ///
    /// The opcode supplies the command name and the body is a compact
    /// varint-length list of arguments. This lets SCNP clients avoid
...
fn take_u32(body: &[u8], cursor: &mut usize, field: &str) -> Result<u32> {
    if body.len().saturating_sub(*cursor) < 4 {
        return Err(ShardCacheError::Protocol(format!("{field} is truncated")));
    }
