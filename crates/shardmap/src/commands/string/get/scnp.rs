...
#[cfg(feature = "server")]
impl ScnpDirectCommand for Get {
    #[inline(always)]
    fn opcode(&self) -> u8 {
        1
    }
    ...
    fn try_execute_scnp(&self, ctx: ScnpCommandContext<'_, '_, '_, '_>) -> ScnpDispatch {
        match GetScnpPath::select(&ctx) {
            GetScnpPath::Unsupported => ScnpDispatch::Unsupported,
            GetScnpPath::FullKeyTaggedOwnedShard => {
                Self::try_execute_full_key_tagged_owned_shard(ctx)
            }
            GetScnpPath::FullKeyTagged => Self::try_execute_full_key_tagged(ctx),
            GetScnpPath::FullKey => Self::try_execute_full_key(ctx),
            GetScnpPath::Generic => Self::try_execute_generic(ctx),
        }
    }
}
...
impl GetScnpPath {
    ...
    fn select(ctx: &ScnpCommandContext<'_, '_, '_, '_>) -> Self {
        ...
        match (
            ...
            (false, SCNP_FULL_KEY_TAGGED_FLAGS, true, _, _) => Self::FullKeyTaggedOwnedShard,
            (false, SCNP_FULL_KEY_TAGGED_FLAGS, _, true, true) => Self::FullKeyTagged,
            (false, SCNP_FULL_KEY_FLAGS, _, true, true) => Self::FullKey,
...
impl Get {
    #[inline(always)]
    fn try_execute_full_key(mut ctx: ScnpCommandContext<'_, '_, '_, '_>) -> ScnpDispatch {
    ...
    #[inline(always)]
    fn try_execute_full_key_tagged(mut ctx: ScnpCommandContext<'_, '_, '_, '_>) -> ScnpDispatch {
    ...
    fn try_execute_full_key_tagged_owned_shard(
        mut ctx: ScnpCommandContext<'_, '_, '_, '_>,
    ) -> ScnpDispatch {
        const BODY_PREFIX_LEN: usize = 24;
        const KEY_HASH_OFFSET: usize = 8;
        ...
        match ctx.store.route_mode() == EmbeddedRouteMode::SessionPrefix && key.starts_with(b"s:") {
            true => {
                ...
                match outcome {
                    ...
                    Some(true) => return ScnpDispatch::Complete(ctx.frame.frame_len),
                    None => {}
                }
            }
    ...
    #[inline(always)]
    fn try_execute_generic(mut ctx: ScnpCommandContext<'_, '_, '_, '_>) -> ScnpDispatch {
    ...
    fn fast_get_value_into(
        ctx: &mut ScnpCommandContext<'_, '_, '_, '_>,
        key_hash: u64,
        key: &[u8],
    ) -> bool {
        let store = ctx.store;
    ...
    #[inline(always)]
    fn write_fast_value(ctx: &mut ScnpCommandContext<'_, '_, '_, '_>, value: &bytes::Bytes) {
