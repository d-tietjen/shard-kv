...
async fn resp_supported_commands_match_redis_reference() {
    let Some(reference) = start_reference_server().await else {
        eprintln!(
            "skipping Redis compatibility differential test: set SHARDCACHE_COMPAT_SERVER_BIN or install valkey-server/redis-server"
        );
        return;
    };
...
async fn run_differential(reference: ReferenceServer) {
    let temp_dir = tempfile::TempDir::new().expect("shardcache temp dir");
    let mut config = test_config(temp_dir.path().join("shardcache-data"), false);
    ...
    let mut shardcache = TcpStream::connect(&config.bind_addr)
        .await
        .expect("connect shardcache");
    ...
    for case in compat_cases() {
        ...
        let shardcache_reply = send_command(&mut shardcache, &parts).await;
        let redis_reply = send_command(&mut redis, &parts).await;
        assert_compatible(&case, shardcache_reply, redis_reply);
    }
...
fn compat_cases() -> Vec<CompatCase> {
    vec![
        ...
        case("EVAL return bulk", &[b"EVAL", b"return 'ok'", b"0"]),
        case(
            "EVAL keys argv",
            &[
                b"EVAL",
        ...
        case("SCRIPT LOAD", &[b"SCRIPT", b"LOAD", b"return 'ok'"]),
        case(
            "SCRIPT EXISTS",
            &[
                b"SCRIPT",
        ...
        case("GETRANGE full", &[b"GETRANGE", b"s", b"0", b"-1"]),
        case("GETRANGE", &[b"GETRANGE", b"s", b"1", b"-1"]),
        case("SUBSTR", &[b"SUBSTR", b"s", b"1", b"-1"]),
        ...
        case("GET hash wrongtype", &[b"GET", b"h"]),
        ...
        case("RPUSH brpl", &[b"RPUSH", b"brpl", b"a", b"b"]),
        case(
            "BRPOPLPUSH ready",
            &[b"BRPOPLPUSH", b"brpl", b"brpl-dst", b"0"],
        ),
        case("LRANGE brpl-dst", &[b"LRANGE", b"brpl-dst", b"0", b"-1"]),
