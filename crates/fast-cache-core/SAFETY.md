...
`fast-cache` keeps a small number of reviewed `unsafe` hot paths for the server
...
```bash
cargo test -p fast-cache-core
cargo test -p fast-cache-core --features unsafe
cargo clippy -p fast-cache-core --features unsafe,experimental-no-ttl-point-hot-path,telemetry,server --all-targets --no-deps
```
...
The FCNP unaligned integer readers in `server::read_le_u32_at` and
