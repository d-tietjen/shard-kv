...
pub(crate) fn flags(route: Option<FcnpRoute>) -> u8 {
    route.map_or(FAST_FLAG_KEY_HASH, |_| ROUTED_FLAGS)
}

pub(crate) fn key_body_len(route: Option<FcnpRoute>, key_len: usize) -> usize {
    route_prefix_len(route) + 4 + key_len
}

pub(crate) fn ttl_key_body_len(route: Option<FcnpRoute>, key_len: usize) -> usize {
...
fn route_prefix_len(route: Option<FcnpRoute>) -> usize {
    if route.is_some() { 20 } else { 8 }
}

fn write_route_prefix<W: Write>(w: &mut W, route: Option<FcnpRoute>, key: &[u8]) -> Result<()> {
