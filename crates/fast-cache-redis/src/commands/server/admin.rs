...
error_command!(
    Monitor,
    "MONITOR",
    "ERR MONITOR is not supported by fast-cache"
);
error_command!(
    Shutdown,
    "SHUTDOWN",
    "ERR SHUTDOWN is disabled by fast-cache"
);
error_command!(
    Sync,
    "SYNC",
    "ERR SYNC replication is not supported by fast-cache"
);
    ...
    fn execute(_store: &EmbeddedStore, args: &[&[u8]]) -> Frame {
        match args {
            [sub] if eq_ignore_ascii_case(sub, b"HELP") => Frame::Array(vec![
                ...
                bulk(b"DEBUG is intentionally limited in fast-cache".to_vec()),
            ]),
            [_sub, ..] => error("ERR DEBUG command is disabled by fast-cache"),
            _ => wrong_arity("DEBUG"),
        }
    }
}
    ...
    fn execute(_store: &EmbeddedStore, args: &[&[u8]]) -> Frame {
        match args {
            ...
            [sub] if eq_ignore_ascii_case(sub, b"DOCTOR") => {
                bulk(b"No latency spikes observed by fast-cache.\n".to_vec())
            }
            [sub] if eq_ignore_ascii_case(sub, b"HELP") => Frame::Array(vec![
...
impl crate::commands::redis::RedisCommand for Lolwut {
    fn execute(_store: &EmbeddedStore, _args: &[&[u8]]) -> Frame {
        bulk(b"fast-cache Redis compatibility\n".to_vec())
    }
}

impl crate::commands::redis::RedisCommand for Migrate {
    fn execute(_store: &EmbeddedStore, _args: &[&[u8]]) -> Frame {
        error("ERR MIGRATE is not supported by fast-cache")
    }
}
    ...
    fn execute(_store: &EmbeddedStore, args: &[&[u8]]) -> Frame {
        match args {
            ...
            [sub] if eq_ignore_ascii_case(sub, b"LOAD") || eq_ignore_ascii_case(sub, b"UNLOAD") => {
                error("ERR Redis modules are not supported by fast-cache")
            }
    ...
    fn execute(_store: &EmbeddedStore, args: &[&[u8]]) -> Frame {
        match args {
            [_replication_id, _offset] => {
                error("ERR PSYNC replication is not supported by fast-cache")
            }
            _ => wrong_arity("PSYNC"),
        }
    }
...
impl crate::commands::redis::RedisCommand for ReplicaOf {
    fn execute(_store: &EmbeddedStore, args: &[&[u8]]) -> Frame {
        match args {
            ...
            [_host, _port] => error("ERR replication is not supported by fast-cache"),
        ...
        while index < tail.len() {
            ...
            if eq_ignore_ascii_case(option, b"ASC") {
                ...
                if !eq_ignore_ascii_case(pattern, b"nosort") {
                    return error("ERR SORT BY patterns are not supported by fast-cache");
                }
                index += 2;
            } else if eq_ignore_ascii_case(option, b"GET") {
                return error("ERR SORT GET patterns are not supported by fast-cache");
            } else {
                return error("ERR syntax error");
            }
        }
...
fn security_warning() -> Frame {
    error("ERR possible SECURITY ATTACK detected. This command is not accepted by fast-cache.")
}
