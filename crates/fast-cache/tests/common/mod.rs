...
use fast_cache::config::{FastCacheConfig, PersistenceConfig, TierConfig};
use fast_cache::protocol::{Frame, RespCodec};
use fast_cache::server::FastCacheServer;
use fast_cache::storage::{Command, EngineHandle, hash_key, shift_for, stripe_index};
...
pub fn test_config(data_dir: PathBuf, persistence: bool) -> FastCacheConfig {
    FastCacheConfig {
...
pub async fn start_server(
    ...
    oneshot::Sender<()>,
    tokio::task::JoinHandle<fast_cache::Result<()>>,
) {
    let engine = EngineHandle::open(config.clone()).expect("engine");
    let server = FastCacheServer::new(config.clone(), engine);
