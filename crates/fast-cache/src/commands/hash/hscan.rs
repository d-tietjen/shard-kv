...
define_redis_command!(HScan, "HSCAN", false);

impl crate::commands::redis::RedisCommand for HScan {
    ...
    fn write_resp(store: &EmbeddedStore, args: &[&[u8]], out: &mut BytesMut) {
        if args.len() < 2 {
            write_frame(out, &wrong_arity("HSCAN"));
            return;
        }
        let outcome = store.hgetall_visit(args[0], |item| write_scan_object_array_item(out, item));
