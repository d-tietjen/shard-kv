...
#[pyclass(name = "ScnpStore")]
pub(crate) struct PyScnpStore {
    addr: String,
    clients: DashMap<ThreadId, Arc<Mutex<ShardCacheClient>>>,
}
...
impl PyScnpStore {
    ...
    fn delete(&self, _key: Vec<u8>) -> PyResult<bool> {
        Err(PyValueError::new_err(
            "SCNP TCP LMCache adapter does not support DELETE yet",
        ))
    }
}
...
impl PyScnpStore {
    fn with_client<T>(
        &self,
        f: impl FnOnce(&mut ShardCacheClient) -> shardcache_client_rs::Result<T>,
    ) -> PyResult<T> {
        let client = self.client_for_thread()?;
        let mut guard = client
            .lock()
            .map_err(|_| PyRuntimeError::new_err("SCNP client mutex poisoned"))?;
        f(&mut guard).map_err(scnp_error_to_py)
    }

    fn client_for_thread(&self) -> PyResult<Arc<Mutex<ShardCacheClient>>> {
        ...
        let client = Arc::new(Mutex::new(
            ShardCacheClient::connect(self.addr.as_str()).map_err(scnp_error_to_py)?,
        ));
...
fn reject_ttl(ttl: Option<u64>) -> PyResult<()> {
    if ttl.is_some() {
        return Err(PyValueError::new_err(
            "SCNP TCP LMCache adapter does not support TTL",
        ));
    }
    Ok(())
}
...
fn pipeline_set_items(
    client: &mut ShardCacheClient,
    items: &[(Vec<u8>, Vec<u8>)],
) -> shardcache_client_rs::Result<()> {
    if items.is_empty() {
        return Ok(());
    }
    if items.len() == 1 {
...
fn pipeline_get_keys(
    client: &mut ShardCacheClient,
    keys: &[Vec<u8>],
) -> shardcache_client_rs::Result<Vec<Option<Vec<u8>>>> {
    if keys.is_empty() {
        return Ok(Vec::new());
    }
    if keys.len() == 1 {
...
fn scnp_error_to_py(error: ShardCacheClientError) -> PyErr {
