use shardmap::cuda::{CudaConfig, CudaSessionTransferRequest};
use shardmap::storage::LocalEmbeddedStore;
...
#[cfg(test)]
mod tests {
    use shardmap::cuda::{CudaChunkTransferDescriptor, CudaSessionTransferRequest};
    ...
    fn connector_restores_to_cpu_target() {
        ...
        let connector = DirectKvConnector::new(shardmap::cuda::CudaConfig::default());
    ...
    fn connector_can_fallback_from_gpu_to_cpu() {
        ...
        let mut dst = vec![0u8; 8];
        let config = shardmap::cuda::CudaConfig {
            enabled: false,
    ...
    fn connector_can_execute_real_cuda_restore_when_enabled() {
        if std::env::var_os("SHARDCACHE_RUN_REAL_CUDA_TESTS").is_none() {
            eprintln!(
                "skipping real CUDA connector smoke test; set SHARDCACHE_RUN_REAL_CUDA_TESTS=1 to enable"
            );
            return;
        }
    ...
    fn connector_can_execute_real_cuda_restore_with_pinned_staging_when_enabled() {
        if std::env::var_os("SHARDCACHE_RUN_REAL_CUDA_STAGING_TESTS").is_none() {
            eprintln!(
                "skipping real CUDA pinned-staging smoke test; set SHARDCACHE_RUN_REAL_CUDA_STAGING_TESTS=1 to enable"
            );
            return;
        }
        ...
        let policy = HostTransferPolicy::from(&shardmap::cuda::CudaConfig {
            prefer_direct_host_dma: false,
            pinned_staging_threshold_bytes: 1,
            ..shardmap::cuda::CudaConfig::default()
        });
        let mut staging_pool = CudaPinnedStagingPool::default();
    ...
    fn connector_can_reuse_real_cuda_direct_dma_engine_when_enabled() {
        if std::env::var_os("SHARDCACHE_RUN_REAL_CUDA_TESTS").is_none() {
            eprintln!(
                "skipping real CUDA direct-DMA reuse smoke test; set SHARDCACHE_RUN_REAL_CUDA_TESTS=1 to enable"
            );
            return;
        }
    ...
    fn real_cuda_benchmark_enabled() -> bool {
        std::env::var_os("SHARDCACHE_RUN_REAL_CUDA_BENCH_TESTS").is_some()
    }
    ...
    fn report_real_cuda_benchmark_case(
        ...
        if !real_cuda_benchmark_enabled() {
            eprintln!(
                "skipping real CUDA benchmark smoke test; set SHARDCACHE_RUN_REAL_CUDA_BENCH_TESTS=1 to enable"
            );
            return;
        }
        ...
        let chunk_size = env_usize("SHARDCACHE_CUDA_BENCH_CHUNK_SIZE_BYTES", 2 * 1024 * 1024);
        let chunk_count = env_usize("SHARDCACHE_CUDA_BENCH_CHUNK_COUNT", 64);
        let warmup_rounds = env_usize("SHARDCACHE_CUDA_BENCH_WARMUP_ROUNDS", 2);
        let measure_rounds = env_usize("SHARDCACHE_CUDA_BENCH_MEASURE_ROUNDS", 6);
        let fragment_gap_bytes = layout.gap_bytes(chunk_size);
