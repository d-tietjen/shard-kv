...
use fast_cache_crate::storage::{CacheMetricsSnapshot, CacheTelemetry};
extern crate fast_cache_runtime as fast_cache_runtime_crate;
#[cfg(feature = "gpu-direct-api")]
use fast_cache_runtime_crate::GpuDirectProxy;
use fast_cache_runtime_crate::{
...
struct ThreadedStoreWorkerState {
    ...
    pending_vllm_restores: FastHashMap<u64, fast_cache_runtime::VllmRestoreHandle>,
...
impl ThreadedStoreWorkerState {
    ...
    fn insert_pending_vllm_restore(
        &mut self,
        handle: fast_cache_runtime::VllmRestoreHandle,
    ) -> u64 {
    ...
    fn take_pending_vllm_restore(
        &mut self,
        pending_id: u64,
    ) -> Option<fast_cache_runtime::VllmRestoreHandle> {
    ...
    fn pending_vllm_restore_ready(
        &mut self,
        pending_id: u64,
    ) -> Result<bool, fast_cache_runtime::RuntimeError> {
        let handle = self
            .pending_vllm_restores
            .get_mut(&pending_id)
            .ok_or_else(|| {
                fast_cache_runtime::RuntimeError::Engine(format!(
    ...
    ) -> Result<DirectVllmRestoreReport, fast_cache_runtime::RuntimeError> {
        let handle = self
            .pending_vllm_restores
            .get_mut(&pending_id)
            .ok_or_else(|| {
...
impl ThreadedStoreWorker {
        ...
        F: FnOnce(&mut ThreadedStoreWorkerState) -> R + Send + 'static,
    {
        ...
        self.tx
            ...
            .expect("fast-cache worker thread is unavailable");
        result_rx
    }
        ...
        F: FnOnce(&mut ThreadedStoreWorkerState) -> R + Send + 'static,
    {
        self.run_async(f)
            .recv()
            .expect("fast-cache worker thread exited before returning a result")
    }
        ...
        V: Into<MutationBytes>,
    {
        if let Some(appender) = self.wal_appenders.get(shard_id) {
            ...
            appender
                ...
                .expect("fast-cache WAL append failed");
        }
    }
...
fn build_persistence_setup(
    ...
    let mut config = FastCacheConfig {
            ...
            Self::GPU_DIRECT_API_V0 => {
                ...
                #[cfg(not(feature = "gpu-direct-api"))]
                {
                    Err(PyValueError::new_err(
                        "path_version='gpu_direct_api_v0' requires fast-cache-py built with feature 'gpu-direct-api'",
                    ))
                }
            }
...
fn direct_vllm_restore_report_from_runtime(
    report: fast_cache_runtime::VllmRestoreReport,
    path_version: DirectVllmRestorePathVersion,
...
impl StoreCore {
    ...
    fn new(
        ...
        if enable_metrics {
            return Err(PyValueError::new_err(
                "fast-cache-py was built without the telemetry feature",
            ));
        }
    ...
    ) -> Result<DirectVllmRestoreTicket, fast_cache_runtime::RuntimeError> {
        ...
        match path_version {
            ...
            DirectVllmRestorePathVersion::GpuDirectApiV0 => {
                ...
                #[cfg(not(feature = "gpu-direct-api"))]
                {
                    ...
                    let _ = cuda;
                    let _ = cpu_fallback;
                    Err(fast_cache_runtime::RuntimeError::Engine(
                        "path_version='gpu_direct_api_v0' requires fast-cache-py built with feature 'gpu-direct-api'".into(),
                    ))
                }
            }
        }
    ...
    fn submit_vllm_paged_restore_host_direct(
        ...
        match self {
            Self::Shared(_) => Err(fast_cache_runtime::RuntimeError::Engine(
    ...
    fn try_wait_vllm_paged_restore(
        &self,
        ticket: DirectVllmRestoreTicket,
    ) -> Result<Option<DirectVllmRestoreReport>, fast_cache_runtime::RuntimeError> {
        match self {
            Self::Shared(_) => Err(fast_cache_runtime::RuntimeError::Engine(
                "direct vLLM restore requires client_architecture='local_embedded'".into(),
            )),
        ...
        match self {
            ...
            Self::Threaded(store) => {
                ...
                for (worker_id, rx, wal_records) in pending {
                    rx.recv()
                        .expect("fast-cache worker thread exited before publishing session batch");
...
impl PyStore {
    ...
    ///
    /// This is a specialized hot path for the fast-cache LMCache backend: it
...
#[pymodule]
fn fast_cache(_py: Python<'_>, module: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    module.add_class::<fcnp_store::PyFcnpStore>()?;
    module.add_function(wrap_pyfunction!(py_hash_key, module)?)?;
    Ok(())
}

fn runtime_error_to_py(error: fast_cache_runtime::RuntimeError) -> PyErr {
