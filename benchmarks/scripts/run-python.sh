#!/usr/bin/env bash
# Python harness runner for fc-py and (optionally) shardcache-lmcache.
#
#   ./scripts/run-python.sh                  # fc-py only
#   LMCACHE=1 ./scripts/run-python.sh        # also runs shardcache-lmcache (lmcache must be installed)
#
...
if [[ "${LMCACHE:-0}" == "1" ]]; then
  if ! python3 -c "import lmcache" 2>/dev/null; then
    echo "lmcache not installed; skipping shardcache-lmcache (pip install lmcache)"
  else
    ...
    python3 "$root/python/shardcache_lmcache_bench.py" \
