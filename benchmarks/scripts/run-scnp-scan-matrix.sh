#!/usr/bin/env bash
# Run the SCNP SCAN benchmark against shardcache's native protocol paths.
...
fanout_addr="${SCNP_FANOUT_ADDR:-127.0.0.1:6383}"
direct_addr="${SCNP_DIRECT_ADDR:-127.0.0.1:6384}"
direct_base_port="${direct_addr##*:}"
shard_count="${SHARD_COUNT:-4}"

cargo build --release -p shardcache --features redis-server --bin shardcache
cargo build --release -p shardcache-benchmarks --bin scnp_scan_matrix
report_pinning

if [[ -n "${SERVER_CPUSET:-}" ]] && command -v taskset >/dev/null 2>&1; then
  env SHARDCACHE_DIRECT_SHARD_PORTS=1 SHARDCACHE_DIRECT_SHARD_BASE_PORT="$direct_base_port" \
    taskset -c "$SERVER_CPUSET" "$ws_root/target/release/shardcache" \
    --bind-addr "$fanout_addr" \
    --shard-count "$shard_count" \
    --disable-persistence \
    --server-mode direct \
    >/tmp/shardcache.scnp-scan-matrix.log 2>&1 &
else
  env SHARDCACHE_DIRECT_SHARD_PORTS=1 SHARDCACHE_DIRECT_SHARD_BASE_PORT="$direct_base_port" \
...
"$ws_root/target/release/scnp_scan_matrix" \
  ...
  --csv "${CSV:-$root/results/scnp-scan-matrix.csv}"
