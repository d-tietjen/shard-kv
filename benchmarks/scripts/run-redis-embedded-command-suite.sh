...
Options:
  ...
  --suite LIST               Suites: redis-core,redis-v6-v7,redis-keyspace,redis-destructive,redis-modules,all
...
expand_suites_raw() {
  ...
  for suite in "${suites_ref[@]}"; do
    ...
    case "$suite" in
      all)
        expanded+=(redis-core redis-v6-v7 redis-keyspace redis-destructive redis-modules)
        ;;
      "")
        ;;
      *)
