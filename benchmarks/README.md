...
| Redis Cluster routing vs shardcache routed server paths | [`REDIS_CLUSTER_SCALABILITY_BENCHMARKS.md`](REDIS_CLUSTER_SCALABILITY_BENCHMARKS.md) | Server Redis Cluster RESP vs shardcache native direct-shard client, shared-port SCNP, and shardcache RESP GET/SET sweep with isolated small, 1 KiB, 4 KiB, 16 KiB, 64 KiB, and 256 KiB value-size suites | Publishable for the saved 16-vCPU direct-SCNP and RESP value-size run; rerun the updated suites to include shared-port SCNP rows or a full 1,2,4,8,16 vCPU matrix. |
