...
For cluster scalability comparisons, include `redis-cluster`,
`shardcache-scnp-direct`, and `shardcache-scnp` with `--key-shards vcpus`. The
Redis Cluster harness uses deterministic hash tags and direct node connections.
`shardcache-scnp-direct` measures client-routed shard ownership, while
`shardcache-scnp` measures the same multi-vCPU shard count through one shared
SCNP server port without client direct routing.
...
| Redis Cluster routing vs shardcache routed server paths | [`REDIS_CLUSTER_SCALABILITY_BENCHMARKS.md`](REDIS_CLUSTER_SCALABILITY_BENCHMARKS.md) | Server Redis Cluster RESP vs shardcache native direct-shard client, shared-port SCNP, and shardcache RESP GET/SET sweep with isolated small, 1 KiB, 4 KiB, 16 KiB, 64 KiB, and 256 KiB value-size suites | Publishable for the saved 16-vCPU direct-SCNP, shared-port SCNP companion, and RESP value-size runs; rerun the full four-target recipe for same-run comparison on new hardware. |
...
| 0.7 active-active pre-implementation baseline | [`ACTIVE_ACTIVE_0_7_BASELINE.md`](ACTIVE_ACTIVE_0_7_BASELINE.md) | Adam raw-cache versus native-map baseline, current replication-cost proxy, LRU cost, and required 0.7 matrix | Baseline only. The active-active runtime is not implemented, so these numbers must not be presented as active-active results. |
