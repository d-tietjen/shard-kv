//! FCNP SCAN benchmark for the native fast-cache protocol.
//!
//! This complements `redis_command_matrix`: it exercises the FCNP wrapper
...
use fcnp_client_rs::{FcnpClient, FcnpDirectRouter, FcnpDirectShardClient};

type BoxError = Box<dyn Error + Send + Sync>;

#[derive(Parser, Debug)]
#[command(about = "Head-to-head FCNP SCAN benchmark")]
struct Args {
    /// Fanout FCNP/RESP listener, usually the server bind address.
    #[arg(long, default_value = "127.0.0.1:6383")]
    fanout_addr: String,

    /// First direct shard FCNP listener.
...
fn main() -> Result<(), BoxError> {
    ...
    let router = FcnpDirectRouter::new(args.direct_addr.as_str(), args.shard_count)?;

    println!(
        "fcnp-scan-matrix: fanout={} direct={} modes={} clients={} keys={} count={} warmup={}s duration={}s",
        args.fanout_addr,
        args.direct_addr,
        args.modes
            .iter()
...
fn seed_keys(addr: &str, key_count: usize, value_size: usize) -> Result<(), BoxError> {
    let mut client = FcnpClient::connect(addr)?;
...
fn run_mode(
    ...
    for handle in handles {
        total.add(&handle.join().map_err(|_| "FCNP scan worker panicked")??);
    }
    Ok(total)
}
...
enum ScanWorker {
    Generic(FcnpClient),
    ShardFanout { client: FcnpClient, shard_id: usize },
    ShardDirect(FcnpDirectShardClient),
}
...
impl ScanWorker {
    fn connect(
        ...
        Ok(match mode {
            ScanMode::Generic => Self::Generic(FcnpClient::connect(fanout_addr)?),
            ScanMode::ShardFanout => Self::ShardFanout {
                client: FcnpClient::connect(fanout_addr)?,
                shard_id,
            },
