# shardmap Embedded Release Benchmarks

Standalone Linux benchmark report for the embedded shardmap release
candidate.
...
- Large-value shardmap GET GB/s uses `--read-mode ref` unless noted. That is
...
- The owner-local embedded path is the clear shardmap strength. It reaches
...
- The 64KiB LRU write-only row is the worst case for shardmap because it
...
- On the same 64KiB LRU shape at 16 workers, read-only reaches `756.93M ops/s`
  for shardmap embedded direct versus `3.31M ops/s` for Moka and `579.8k
  ops/s` for the `lru` crate. The 80/20 row is close to Moka: `1.45M ops/s`
  for shardmap versus `1.42M ops/s` for Moka.
- On the smaller 4KiB LRU shape at 16 workers, shardmap embedded direct
...
below isolates write-only behavior because that is the worst shardmap case
...
Large-value shardmap GET rows use the default `--read-mode ref` behavior:
...
logical payload rate for shardmap reference-read rows, not real data
...
| Mix | shardmap embedded direct | shardmap shared handle | Moka | `lru` crate |
| --- | ---: | ---: | ---: | ---: |
...
| --- | --- | --- | --- | ---: | ---: | ---: |
| none | 1/1 | shardmap embedded direct | safe | 186,626 | 12.231 | 1.000 |
| none | 1/1 | shardmap embedded direct | unsafe | 187,176 | 12.267 | 1.000 |
| none | 1/1 | shardmap shared handle | safe | 183,801 | 12.046 | 1.000 |
| none | 1/1 | Moka | competitor | 239,218 | 15.677 | 1.000 |
| none | 1/1 | `lru` crate | competitor | 176,198 | 11.547 | 1.000 |
| active | 1/1 | shardmap embedded direct | safe | 180,478 | 11.828 | 1.000 |
| active | 1/1 | shardmap embedded direct | unsafe | 180,248 | 11.813 | 1.000 |
| active | 1/1 | shardmap shared handle | safe | 179,329 | 11.752 | 1.000 |
| active | 1/1 | Moka | competitor | 238,049 | 15.601 | 1.000 |
| none | 16/16 | shardmap embedded direct | safe | 291,721 | 19.118 | 15.995 |
| none | 16/16 | shardmap embedded direct | unsafe | 291,661 | 19.114 | 15.985 |
| none | 16/16 | shardmap shared handle | safe | 288,029 | 18.876 | 9.696 |
| none | 16/16 | Moka | competitor | 511,055 | 33.492 | 9.239 |
| none | 16/16 | `lru` crate | competitor | 140,631 | 9.216 | 1.590 |
| active | 16/16 | shardmap embedded direct | safe | 288,418 | 18.902 | 15.991 |
| active | 16/16 | shardmap embedded direct | unsafe | 291,181 | 19.083 | 15.993 |
| active | 16/16 | shardmap shared handle | safe | 285,668 | 18.722 | 9.566 |
...
Medians for shardmap embedded direct, write-only LRU.
