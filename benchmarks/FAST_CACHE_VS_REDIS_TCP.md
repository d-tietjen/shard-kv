# fast-cache vs Redis, Valkey, and Dragonfly over TCP

Published: 2026-05-14
Updated: 2026-05-18

This note collects the Linux benchmark runs that compare fast-cache over
...
The same-core view pins fast-cache to one CPU and starts it with
`--shard-count 1`. Redis is single-threaded, so this is the per-core comparison.

The 16-vCPU view pins fast-cache to CPUs `0-15` and starts it with
`--shard-count 16`. Redis remains the single-threaded TCP baseline. This shows
the fast-cache scale-out shape against a drop-in Redis deployment.
...
## server Large Network Database Matrix, 2026-05-18
...
| Value | FCNP direct | fast-cache RESP | Redis | Valkey | Dragonfly |
| ---: | ---: | ---: | ---: | ---: | ---: |
...
| Mix | FCNP direct | fast-cache RESP | Redis | Valkey | Dragonfly |
| --- | ---: | ---: | ---: | ---: | ---: |
...
| Value | Mix | FCNP direct | fast-cache RESP | Redis | Valkey | Dragonfly |
| ---: | --- | ---: | ---: | ---: | ---: | ---: |
...
| --- | ---: |
| FCNP direct | 894k / 0.06 @ 13.9 (256c, p1) |
| fast-cache RESP | 888k / 0.06 @ 14.1 (256c, p1) |
| Redis | 91k / 0.01 @ 1.0 (16c, p1) |
| Valkey | 89k / 0.01 @ 1.0 (256c, p1) |
| Dragonfly | 757k / 0.05 @ 16.0 (256c, p1) |

For the `64B 80/20` best-throughput row, FCNP direct is `26.2x` Redis,
fast-cache RESP is `16.6x` Redis, Valkey is effectively tied with Redis, and
...
At pipeline depth `1`, same-core fast-cache is modestly ahead of Redis. With
16 server cores available, fast-cache is roughly 10x Redis for this workload
because requests can be routed across the shard-owned workers.

## Same-Core Non-Pipelined Matrix

Fast-cache: `taskset -c 0`, `--shard-count 1`. Redis: single-threaded baseline.
Pipeline depth: `1`.
...
Fast-cache: `taskset -c 0-15`, `--shard-count 16`. Redis: single-threaded
...
Fast-cache: `taskset -c 0`, `--shard-count 1`.
...
| Target | FCNP direct | fast-cache RESP | Redis |
| ---: | ---: | ---: | ---: |
...
uses for the same delivered request rate. At the 2M target, FCNP direct still
has headroom on one server CPU, fast-cache RESP is near the edge of one CPU,
and Redis saturates at about `1.23M` ops/sec.

### 16-vCPU Pipeline Sweep

Fast-cache: `taskset -c 0-15`, `--shard-count 16`.
...
At depth `64`, FCNP direct reaches 33.85M ops/sec under the 16-vCPU cap while
...
The `p64` sweep is not the single-core FCNP ceiling. The direct-shard client
continues to improve with deeper queues, so this pass includes a standalone
deeper-pipeline check for the same-core FCNP direct path.

Fast-cache: FCNP direct, `taskset -c 0`, `--shard-count 1`, `64B`, `64`
clients, `100k` keys.

| Pipeline depth | Mix | FCNP direct |
| ---: | --- | ---: |
...
| Clients | FCNP direct | RESP | Redis |
| ---: | ---: | ---: | ---: |
...
The knee is around `16` clients for fast-cache in this run. More clients do
...
`--disable-persistence` for fast-cache. Some fast-cache rows report more than
...
```bash
BACKENDS=fc-server-fcnp-direct ADDR=127.0.0.1:6501 \
...
- Non-pipelined same-core TCP is close: fast-cache is ahead, but by a modest
  margin.
- Non-pipelined 16-vCPU fast-cache is roughly 10x Redis on the 64B 80/20 row.
- Pipelining is the critical TCP dimension. At depth `64`, FCNP direct reaches
  33.85M ops/sec under the 16-vCPU cap, while Redis reaches 1.16M ops/sec.
- At a fixed 1M ops/sec same-core pipelined load, FCNP direct uses 0.439 vCPU
  versus Redis at 0.909 vCPU. At a 2M target, FCNP direct still completes the
  row at 0.843 vCPU while Redis saturates at 1.23M ops/sec.
- Deep pipeline FCNP direct reaches 5.52M ops/sec on the same-core 80/20 row.
- In the single-core client sweep, the useful client-count knee is around `16`
  clients for the pipelined fast-cache rows; `64` and `256` clients do not add
  throughput.
...
Fast-cache same-core:

```bash
taskset -c 0 ./target/release/fast-cache-server \
...
```bash
taskset -c 0-15 ./target/release/fast-cache-server \
  --server-mode direct --disable-persistence \
  --bind-addr 127.0.0.1:6383 \
  --shard-count 16
```

FCNP direct shard ports:

```bash
FAST_CACHE_USE_MONOIO=1 FAST_CACHE_DIRECT_SHARD_PORTS=1 \
  taskset -c 0 ./target/release/fast-cache-server \
  --server-mode direct --disable-persistence \
...
unless `FAST_CACHE_DIRECT_SHARD_BASE_PORT` is set.
...
```bash
cargo run --release -p fast-cache-benchmarks --bin curve -- \
  --backends fc-server-fcnp-direct \
  --addr 127.0.0.1:6501 \
