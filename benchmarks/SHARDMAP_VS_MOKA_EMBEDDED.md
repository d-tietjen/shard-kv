# shardmap vs Moka Embedded

This report records a local embedded head-to-head between shardmap's
...
| Online CPUs | 10 |
| Driver | `shardcache-benchmarks` `saturation` |
...
`fc-embed` measures the intended shardmap embedded architecture: each worker
...
buffer. The shardmap `fc-embed` GET path checks a local point reference and
...
`1MiB`, `10` clients, shardmap and Moka are close on pure SET throughput
(`119.6K` ops/s vs `116.1K` ops/s).

## Summary

shardmap won all `45/45` paired rows in this run.

| Scope | Result |
| --- | --- |
| Small values, 10 clients | shardmap was `25.9x` to `93.5x` faster depending on size and mix |
| 64KiB, 10 clients | shardmap was `4.1x` faster on SET, `8.8x` on 80/20, and `418.7x` on logical GET |
| 1MiB, 10 clients | shardmap was `1.03x` faster on SET, `6.45x` on 80/20, and `6002.9x` on logical GET |
| Tail latency | shardmap p99 was lower in every 10-client row |

## 10-Client Headline

| Value | Mix | shardmap ops/s | Moka ops/s | Speedup | shardmap GB/s | Moka GB/s | shardmap vCPU | Moka vCPU |
| ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
| Value | Mix | shardmap p99 | Moka p99 | shardmap p999 | Moka p999 |
| ---: | --- | ---: | ---: | ---: | ---: |
